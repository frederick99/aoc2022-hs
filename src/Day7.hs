...
partOne = bags >>= \bags -> print
        . subtract 1 . length   -- exclude the shiny gold bag
        . filter (Set.member "shiny gold" . uniqueColors bags)
        $ Map.keys bags
-- Answer: 169

countBags :: Bags -> Color -> Integer
countBags bags color = maybe 0 (foldl' go 0) (Map.lookup color bags)
  where go acc (n, color) = n * (countBags bags color + 1) + acc
